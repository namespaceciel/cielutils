...
TEST(split_buffer_tests, constructors) {
    ...
    const ciel::split_buffer v2(v1);
    ASSERT_TRUE(v2.empty());

    const ciel::split_buffer v3(10, 20);
    ASSERT_EQ(v3.size(), 10);

    const ciel::split_buffer<int> v4(15);
    ASSERT_EQ(v4.size(), 15);

    ciel::split_buffer v5(v4);
    ASSERT_EQ(v5.size(), 15);

    const ciel::split_buffer v6(std::move(v5));
    ASSERT_EQ(v5.size(), 0);
    ASSERT_EQ(v6.size(), 15);

    const ciel::split_buffer v7({1, 2, 3, 4, 5});
    ASSERT_EQ(v7.size(), 5);

    constexpr ciel::split_buffer v8(0, 10);
    ASSERT_TRUE(v8.empty());

    constexpr ciel::split_buffer<int> v9(0);
    ASSERT_TRUE(v9.empty());

    const ciel::split_buffer v10(v7.begin(), v7.begin());
    ASSERT_TRUE(v10.empty());
}

TEST(split_buffer_tests, assignments) {
    ciel::split_buffer v1({1, 2, 3, 4, 5});
    ...
    v3.assign(4, 10);
    ASSERT_EQ(v3, std::initializer_list({10, 10, 10, 10}));
    ...
    v3.assign(7, 10);
    ASSERT_EQ(v3, std::initializer_list({10, 10, 10, 10, 10, 10, 10}));
}
...
TEST(split_buffer_tests, push_and_pop) {
    ...
    ciel::split_buffer v2({0, 1, 2, 3, 4});
...
TEST(split_buffer_tests, resize) {
    ...
    v1.resize(15, 10);
    ASSERT_EQ(v1, std::initializer_list({5, 77, 77, 77, 77, 77, 77, 77, 77, 77, 44, 44, 10, 10, 10}));
    ...
    v1.resize(18, 19);
    ASSERT_EQ(v1, std::initializer_list({5, 77, 77, 77, 77, 77, 77, 77, 77, 77, 44, 44, 10, 10, 10, 19, 19, 19}));
}

TEST(split_buffer_tests, insert_and_emplace) {
    ciel::split_buffer v1{0, 1, 2, 3, 4, 5, 6};
    ...
    ASSERT_EQ(v1, std::initializer_list({22, 18, 41, 99, 99, 99, 99, 41, 21, 0, 1, 2, 3, 4, 5, 42, 43, 6, 19, 31,
                                              32}));
    ...
    ASSERT_EQ(v1, std::initializer_list({22, 18, 16, 41, 99, 99, 99, 99, 41, 21, 0, 1, 2, 3, 4, 5, 42, 43, 6, 19,
                                              31, 17, 32}));
    ...
    ASSERT_EQ(v1, std::initializer_list({22, 18, 65, 65, 65, 16, 41, 99, 99, 99, 99, 41, 21, 0, 1, 2, 3, 4, 5, 42,
                                              43, 6, 19, 31, 17, 32}));
...
TEST(split_buffer_tests, copy_and_move_behavior) {
    ...
    ciel::split_buffer v2(6, ConstructAndAssignCounter{});
    ...
    const ciel::split_buffer v5(v1.begin(), v1.end() - 1);
