...
TEST(vector_tests, insert_and_emplace) {
    ...
    ASSERT_EQ(v1, std::initializer_list({22, 21, 0, 1, 2, 41, 41, 3, 42, 43, 4, 5, 6, 31, 32}));

    // insert when expansion
    v1.shrink_to_fit();
    ASSERT_EQ(*v1.insert(v1.begin() + 2, 99), 99);
    ASSERT_EQ(v1, std::initializer_list({22, 21, 99, 0, 1, 2, 41, 41, 3, 42, 43, 4, 5, 6, 31, 32}));

    // insert self range when expansion
    v1.shrink_to_fit();
    ASSERT_EQ(*v1.insert(v1.begin() + 2, v1.begin() + 1, v1.begin() + 5), 21);
    ASSERT_EQ(v1, std::initializer_list({22, 21, 21, 99, 0, 1, 99, 0, 1, 2, 41, 41, 3, 42, 43, 4, 5, 6, 31, 32}));
}
