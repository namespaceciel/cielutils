...
TEST(list_tests, assign) {
    ciel::list l1{1, 2, 3};
    l1.assign(5, 123);
    ASSERT_EQ(l1, ciel::list({123, 123, 123, 123, 123}));

    l1.assign({432, 53, 1, 67});
    ASSERT_EQ(l1, ciel::list({432, 53, 1, 67}));

    ciel::list l2{654, 433, 21, 987, 655};
    l1.assign(l2.begin(), l2.end());
    ASSERT_EQ(l1, ciel::list({654, 433, 21, 987, 655}));
}
...
TEST(list_tests, insertions) {
    ...
    l1.push_front(1);
    l1.emplace_front(0);
    ASSERT_EQ(l1, ciel::list({0, 1, 2, 3, 4, 5}));

    l1.insert(l1.begin().next(), 123);
    l1.insert(l1.end(), 123);
    ASSERT_EQ(l1, ciel::list({0, 123, 1, 2, 3, 4, 5, 123}));

    l1.insert(l1.end().prev(), 3, 666);
    ASSERT_EQ(l1, ciel::list({0, 123, 1, 2, 3, 4, 5, 666, 666, 666, 123}));

    l1.insert(l1.begin(), {11, 22, 33});
    ASSERT_EQ(l1, ciel::list({11, 22, 33, 0, 123, 1, 2, 3, 4, 5, 666, 666, 666, 123}));

    ciel::list l2{98, 87, 76};
    l1.insert(l1.begin(), l2.begin(), l2.end());
    ASSERT_EQ(l1, ciel::list({98, 87, 76, 11, 22, 33, 0, 123, 1, 2, 3, 4, 5, 666, 666, 666, 123}));

    l1.emplace(l1.begin().next(), 87654);
    ASSERT_EQ(l1, ciel::list({98, 87654, 87, 76, 11, 22, 33, 0, 123, 1, 2, 3, 4, 5, 666, 666, 666, 123}));

    l1.erase(l1.begin());
    ASSERT_EQ(l1, ciel::list({87654, 87, 76, 11, 22, 33, 0, 123, 1, 2, 3, 4, 5, 666, 666, 666, 123}));

    l1.erase(l1.begin(), l1.end().prev());
    ASSERT_EQ(l1, ciel::list({123}));

    l1.resize(5);
    ASSERT_EQ(l1, ciel::list({123, 0, 0, 0, 0}));

    l1.resize(3);
    ASSERT_EQ(l1, ciel::list({123, 0, 0}));

    l1.resize(6, 123);
    ASSERT_EQ(l1, ciel::list({123, 0, 0, 123, 123, 123}));

    l1.pop_back();
    l1.pop_front();
    ASSERT_EQ(l1, ciel::list({0, 0, 123, 123}));
}
...
TEST(list_tests, swap) {
    ...
    std::swap(l1, l2);
    ASSERT_EQ(l1, ciel::list({6, 7, 8, 9, 6, 4, 3}));
    ASSERT_EQ(l2, ciel::list({4, 3, 2, 1}));
}